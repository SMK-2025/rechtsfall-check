...
export default async function LegalAreaPage({ params }: PageProps) {
  ...
  return <div className="site"><StructuredData data={schema}/><SiteHeader/><main>
    <section className="subhero legal-area-hero"><div>
      ...
      <h1>{area.title}: deinen Rechtsfall strukturiert vorprüfen.</h1>
      <p>{area.examples}. Der Rechtsfall Check verbindet deine Schilderung, Unterlagen und gezielte Rückfragen zu einer verständlichen, nicht abschließenden Ersteinschätzung.</p>
      <PublicSignupLink className="button">Zur kostenlosen Registrierung →</PublicSignupLink>
    </div></section>

    <section className="section-wrap legal-area-content"><header><span className="section-label">TYPISCHE FRAGEN</span><h2>Welche Themen können eingeordnet werden?</h2><p>Die Auswahl hilft bei der strukturierten Fallaufnahme. Entscheidend bleiben deine konkreten Angaben, Nachweise und mögliche Fristen.</p></header>
