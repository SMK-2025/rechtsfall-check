...
export const metadata: Metadata = {
  title: "Fragen zum Rechtsfall Check",
  description: "Antworten zu Ablauf, Preis, Rechtsgebieten, Dokumenten, Datenschutz und Leistungsgrenzen.",
...
export default function FaqPage() {
  const faqItems = groups.flatMap(([, items]) => items);
  const schema = {
  ...
  return <div className="site"><StructuredData data={schema}/><SiteHeader/><main>
    <section className="subhero"><div><span className="section-label light-label">HÄUFIGE FRAGEN</span><h1>Alles Wichtige zum Rechtsfall Check.</h1><p>Produkt, Preis, Ablauf und Grenzen verständlich erklärt.</p></div></section>
    <section className="faq-page section-wrap">{groups.map(([title,items])=><div className="faq-group" key={title}><h2>{title}</h2><div className="faq-list">{items.map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div></div>)}</section>
    <ConversionCta/><SiteFooter/>
  </main></div>;
}
