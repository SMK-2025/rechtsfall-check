...
export function ProfileForm({ initial, required = false, returnTo = "/fallraum" }: { initial: Profile; required?: boolean; returnTo?: string }) {
  ...
  async function saveProfile(event: FormEvent) {
    ...
    try {
      ...
      setProfileNotice({ type: "success", text: "Deine persönlichen Angaben wurden gespeichert." });
  ...
  async function changeEmail(event: FormEvent) {
    ...
    if (!newEmail.trim() || newEmail.trim().toLowerCase() === profile.email.toLowerCase()) {
      setEmailNotice({ type: "error", text: "Bitte gib eine andere, gültige E-Mail-Adresse ein." });
      return;
    }
    setEmailBusy(true);
  ...
  async function changePassword(event: FormEvent) {
    ...
    if (error) {
      setPasswordNotice({ type: "error", text: error.message || "Das Passwort konnte nicht geändert werden. Prüfe dein bisheriges Passwort." });
      return;
    }
    setPasswords({ current: "", next: "", confirmation: "" });
    setPasswordNotice({ type: "success", text: "Dein Passwort wurde geändert. Andere aktive Sitzungen wurden aus Sicherheitsgründen beendet." });
  }
  ...
  async function requestDeletion(event: FormEvent) {
    ...
    if (!deletionAcknowledged || deletionConfirmation !== "LÖSCHEN") {
      setDeletionNotice({ type: "error", text: "Bitte bestätige die Folgen und gib LÖSCHEN vollständig ein." });
      return;
    }
    ...
    try {
      ...
      setDeletionNotice({ type: "success", text: "Die Löschung ist vorgemerkt. Du kannst sie bis zum angezeigten Termin widerrufen." });
    } catch (error) {
  ...
  async function cancelDeletion() {
    ...
    try {
      ...
      setDeletionNotice({ type: "success", text: "Die vorgemerkte Kontolöschung wurde widerrufen. Dein Konto bleibt bestehen." });
    } catch (error) {
  ...
  async function withdrawAiConsent() {
    if (!window.confirm("Möchtest du deine KI-Einwilligung für alle Fallakten mit Wirkung für die Zukunft widerrufen? Ohne eine neue ausdrückliche Einwilligung können keine weiteren KI-Analysen durchgeführt werden.")) return;
    setConsentBusy(true);
    ...
    try {
      ...
      setConsentNotice({
        ...
        text: result.affectedCaseCount
          ? `Deine Einwilligung wurde für ${result.affectedCaseCount} Fallakte${result.affectedCaseCount === 1 ? "" : "n"} mit Wirkung für die Zukunft widerrufen.`
  ...
  return <div className="member-shell">
    ...
    <main className="member-main profile-main">
      <div className="member-heading"><div><span className="section-label">MEIN KONTO</span><h1>Persönliche Angaben</h1><p>Verwalte deine Kontakt-, Adress- und Zugangsdaten.</p></div></div>
      {required && <div className="profile-required-notice" role="status"><strong>Bitte vervollständige zuerst dein Profil.</strong><span>Danach kannst du deinen Rechtsfall Check und alle weiteren Kontofunktionen nutzen.</span></div>}

      <form id="persoenliche-daten" className="app-card profile-card" onSubmit={saveProfile}>
        <div className="profile-card-head"><div className="profile-card-icon" aria-hidden="true">01</div><div><h2>Persönliche Daten</h2><p>Diese Angaben werden deinem Nutzerkonto und deinen Fallakten zugeordnet.</p></div></div>
        <div className="app-grid profile-grid">
          ...
          <div className="field"><label htmlFor="accountEmail">E-Mail-Adresse</label><input id="accountEmail" type="email" value={profile.email} readOnly aria-describedby="email-change-hint"/><small className="field-help" id="email-change-hint">Änderungen nimmst du unten im Bereich Kontosicherheit vor.</small></div>
        </div>
        <NoticeBox notice={profileNotice}/>
        <div className="app-actions"><small>Deine Angaben werden verschlüsselt übertragen und nur für dein Konto und die Bearbeitung deines Problems verwendet.</small><button className="button" disabled={profileBusy}>{profileBusy ? "Wird gespeichert …" : "Änderungen speichern"}</button></div>
      </form>

      <section id="zugangsdaten" className="profile-security" aria-labelledby="security-title">
        <div className="profile-section-title"><span className="section-label">KONTOSICHERHEIT</span><h2 id="security-title">Zugangsdaten verwalten</h2><p>E-Mail-Adresse und Passwort werden getrennt von deinen persönlichen Angaben geändert.</p></div>
        <div className="profile-security-grid">
          <form className="app-card profile-card security-card" onSubmit={changeEmail}>
            <div className="profile-card-head"><div className="profile-card-icon" aria-hidden="true">02</div><div><h2>E-Mail-Adresse ändern</h2><p>Deine derzeitige Login-Adresse lautet <strong>{profile.email}</strong>.</p></div></div>
            ...
            <p className="security-hint">Die neue Adresse wird erst aktiv, nachdem du den Link in der Bestätigungs-E-Mail geöffnet haben.</p>
          ...
          <form className="app-card profile-card security-card" onSubmit={changePassword}>
            <div className="profile-card-head"><div className="profile-card-icon" aria-hidden="true">03</div><div><h2>Passwort ändern</h2><p>Wähle ein neues Passwort mit mindestens 10 Zeichen.</p></div></div>
      ...
      <section id="datenschutz" className="profile-security" aria-labelledby="privacy-tools-title">
        <div className="profile-section-title"><span className="section-label">DATENSCHUTZ</span><h2 id="privacy-tools-title">Deine Daten im Blick</h2><p>Lade die zu deinem Konto gespeicherten strukturierten Daten in einem maschinenlesbaren Format herunter.</p></div>
        <div className="app-card profile-card">
          ...
          <div className="app-actions"><small>Der Download wird nur in deiner angemeldeten Sitzung erzeugt und nicht zwischengespeichert.</small><a className="button" href="/api/v1/privacy/export" download>Meine Daten herunterladen</a></div>
        </div>
        <div className="app-card profile-card">
          <div className="profile-card-head"><div className="profile-card-icon" aria-hidden="true">05</div><div><h2>KI-Einwilligung widerrufen</h2><p>Beendet die einwilligungsbasierte KI-Verarbeitung deiner Fallakten mit Wirkung für die Zukunft.</p></div></div>
          <p>Bereits erstellte Rechtsfall-Checks bleiben in deinem Konto gespeichert. Für eine weitere Analyse musst du in der betreffenden Fallakte erneut ausdrücklich einwilligen.</p>
        ...
        <div id="konto-loeschen" className="app-card profile-card account-deletion-card">
          <div className="profile-card-head"><div className="profile-card-icon danger" aria-hidden="true">06</div><div><h2>Konto löschen</h2><p>Entfernt dein Nutzerkonto und sämtliche zugeordneten Rechtsfall-Checks unwiderruflich.</p></div></div>
          {deletionScheduledFor ? <>
            <div className="deletion-status" role="status">
              <strong>Dein Konto ist zur Löschung vorgemerkt.</strong>
              <p>Die endgültige Löschung erfolgt am <b>{new Intl.DateTimeFormat("de-DE", { dateStyle: "long", timeStyle: "short" }).format(new Date(deletionScheduledFor))}</b>. Bis dahin bleibt das Konto aktiv und du kannst die Löschung widerrufen.</p>
            </div>
            ...
            <div className="deletion-warning">
              ...
              <ul><li>Deine persönlichen Daten und Zugangsdaten</li><li>Alle Fallbeschreibungen, Antworten und Rückfragen</li><li>Alle hochgeladenen Dokumente aus dem Dateispeicher</li><li>Alle KI-Analysen, Ergebnisse und Prüfberichte</li><li>Alle Sitzungen und internen Kontoverknüpfungen</li></ul>
            ...
            <fieldset className="deletion-options">
              ...
              <label><input type="radio" name="deletionMode" value="scheduled" checked={deletionMode === "scheduled"} onChange={() => setDeletionMode("scheduled")}/><span><strong>Mit 30 Tagen Widerrufsfrist</strong><small>Das Konto bleibt 30 Tage aktiv. In dieser Zeit kannst du die Löschung jederzeit widerrufen.</small></span></label>
              <label><input type="radio" name="deletionMode" value="immediate" checked={deletionMode === "immediate"} onChange={() => setDeletionMode("immediate")}/><span><strong>Sofort und ohne Widerrufsfrist löschen</strong><small>Du verzichtest ausdrücklich auf die 30-tägige Widerrufsmöglichkeit. Die Löschung beginnt sofort.</small></span></label>
            </fieldset>
