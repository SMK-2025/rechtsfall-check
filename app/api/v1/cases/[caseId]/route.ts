...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const item = await ownedCase(caseId, member.id);
  if (!item || item.status === "DELETED") return apiError("CASE_NOT_FOUND", 404, "Fall nicht gefunden.");
  const body = await request.json() as { title?: string; status?: string; legalArea?: string };
  ...
  await getDb().update(cases).set({ title, status, legalArea, updatedAt: new Date() }).where(and(eq(cases.id, caseId), eq(cases.ownerId, member.id)));
  await writeAudit({ caseId, actorId: member.id, eventType: "CASE_UPDATED", targetType: "case", targetId: caseId, metadata: { status, legalArea } });
  return Response.json({ case: { ...item, title, status, legalArea } }, { headers: { "cache-control": "no-store" } });
}
