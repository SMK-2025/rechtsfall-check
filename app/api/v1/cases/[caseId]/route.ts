...
export async function PATCH(request: Request, { params }: Params) {
  ...
  await getDb().update(cases).set({ title, status, updatedAt: new Date().toISOString() }).where(and(eq(cases.id, caseId), eq(cases.ownerId, member.id)));
...
export async function DELETE(_: Request, { params }: Params) {
  ...
  const item = await ownedCase(caseId, member.id);
  if (!item || item.status === "DELETED") return apiError("CASE_NOT_FOUND", 404, "Fall nicht gefunden.");
  await getDb().update(cases).set({ status: "DELETED", updatedAt: new Date().toISOString() }).where(and(eq(cases.id, caseId), eq(cases.ownerId, member.id)));
