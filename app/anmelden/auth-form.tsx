...
export function AuthForm({ callbackURL, initialMode }: { callbackURL: string; initialMode: "login" | "signup" }) {
  ...
  return <>
    ...
    <form className="auth-form" id="auth-panel" role="tabpanel" onSubmit={submit} onFocus={()=>{if(mode==="signup"&&!signupFormStarted.current){signupFormStarted.current=true;trackAnalyticsEvent("signup_form_started")}}} aria-busy={busy}>
      {mode==="signup"&&<div className="field"><label htmlFor="name">Ihr Name (optional)</label><input id="name" name="name" autoComplete="name" placeholder="Vor- und Nachname"/></div>}
      ...
      <button className="button auth-submit" disabled={busy}>{busy?"Einen Moment …":mode==="signup"?"Kostenlose Fallakte anlegen →":"Login →"}</button>
    </form>
    {mode==="login"&&<Link className="forgot-password-link" href="/passwort-vergessen">Passwort vergessen?</Link>}
    <p className="auth-terms">Kein Abo. Keine Zahlung bei der Registrierung. Der Rechtsfall Check kostet erst bei ausdrücklicher Beauftragung einmalig 19 €.</p>
  </>;
}
