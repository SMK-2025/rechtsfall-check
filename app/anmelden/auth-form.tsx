...
export function AuthForm({ callbackURL, initialMode, initialAccountType = "" }: { callbackURL: string; initialMode: "login" | "signup"; initialAccountType?: AccountType }) {
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    if(mode==="signup"&&!accountType){setError("Bitte wähle aus, ob du dich als Mandant oder Kanzlei registrieren möchtest.");setBusy(false);return}
    ...
    if(result.error){setError(mode==="signup"?(accountType==="LAWYER"?"Konto konnte nicht erstellt werden. Bitte prüfen Sie Ihre Angaben.":"Konto konnte nicht erstellt werden. Bitte prüfe deine Angaben."):"Login nicht möglich. Prüfe deine Zugangsdaten und bestätige gegebenenfalls zuerst deine E-Mail-Adresse.");setBusy(false);return}
    if(mode==="signup"){trackAnalyticsEvent("sign_up",{method:"email"});setNotice(accountType==="LAWYER"?"Bitte prüfen Sie Ihr Postfach. Für ein neues Konto erhalten Sie den Bestätigungslink.":"Bitte prüfe dein Postfach. Für ein neues Konto erhältst du den Bestätigungslink. Besteht bereits ein Konto, senden wir dir stattdessen einen sicheren Hinweis zum Login.");setBusy(false);return}
  ...
  return <>
    ...
    <form className="auth-form" id="auth-panel" role="tabpanel" onSubmit={submit} onFocus={()=>{if(mode==="signup"&&!signupFormStarted.current){signupFormStarted.current=true;trackAnalyticsEvent("signup_form_started")}}} aria-busy={busy}>
      {mode==="signup"&&<fieldset className="account-type-picker"><legend>Wie möchtest du Rechtsfall-Check.de nutzen?</legend><label className={accountType==="MEMBER"?"selected":""}><input type="radio" name="accountType" value="MEMBER" checked={accountType==="MEMBER"} onChange={()=>setAccountType("MEMBER")}/><span><strong>Als Mandant</strong><small>Problem analysieren und Lösungswege erhalten</small></span></label><label className={accountType==="LAWYER"?"selected":""}><input type="radio" name="accountType" value="LAWYER" checked={accountType==="LAWYER"} onChange={()=>setAccountType("LAWYER")}/><span><strong>Als Kanzlei</strong><small>Anwaltsprofil vorbereiten und später am Matching teilnehmen</small></span></label></fieldset>}
      {mode==="signup"&&<div className="field"><label htmlFor="name">{accountType==="LAWYER"?"Ihr Name / Ansprechpartner":"Dein Name (optional)"}</label><input id="name" name="name" autoComplete="name" placeholder="Vor- und Nachname"/></div>}
