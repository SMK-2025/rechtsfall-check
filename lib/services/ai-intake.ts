...
export async function analyzeIntake(input:{legalArea?:string;topic?:string;eventDate?:string;federalState?:string;opposingParty?:string;description?:string;desiredOutcome?:string},safetyIdentifier:string):Promise<AiIntake|null>{
  ...
  const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{authorization:`Bearer ${apiKey}`,"content-type":"application/json"},body:JSON.stringify({
    ...
    input:`Rechtsgebiet: ${input.legalArea||"nicht angegeben"}\nThema: ${input.topic||"nicht angegeben"}\nEreignisdatum: ${input.eventDate||"nicht angegeben"}\nBundesland: ${input.federalState||"nicht angegeben"}\nAndere Seite: ${input.opposingParty||"nicht angegeben"}\nGewünschtes Ergebnis: ${input.desiredOutcome||"nicht angegeben"}\nSchilderung: ${input.description||"nicht angegeben"}`,
